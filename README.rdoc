...
=== Usage
  ...
  digest_num = digest.reverse.unpack('H*')[0].hex
...
=== Convert GOST digest, public_key and signature strings to numbers

  require 'crypto_gost3410'
  require 'crypto_gost3411'

  group = CryptoGost3410::Group::Gost256tc26test  
  # get tbs, signer public_key and signature strings from certificate
  # ...
  # generate digest (32 bytes)
  digest32 = CryptoGost3411::Gost3411.new(32).update(tbs).final
  # little-endian digest32 -> reverse -> big-endian -> BigNum
  digest32_num = digest32.reverse.unpack('H*')[0].hex
  # public key x||y little-endian -> reverse -> big-endian -> BigNums
  public_key_x_num = public_key[0...32].reverse.unpack('H*')[0].hex
  public_key_y_num = public_key[32..-1].reverse.unpack('H*')[0].hex
  public_key_point = CryptoGost3410::Point.new(group, [public_key_x_num, public_key_y_num])
  # signature s||r big-endian -> BigNums
  signature_s_num = signature[0...32].unpack('H*')[0].hex
  signature_r_num = signature[32..-1].unpack('H*')[0].hex
